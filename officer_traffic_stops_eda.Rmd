...
knitr::opts_chunk$set(echo = FALSE, tidy.opts=list(width.cutoff=40),tidy=TRUE)
rm(list=ls())
cat("\014")
```
...
myData %>%
      group_by(Officer_Race, Officer_Gender) %>% 
        na.omit() %>% 
            count %>% 
              arrange(desc(n)) %>% 
                knitr::kable(col.names = c("Officer Race", "Officer Gender", "Count"), format.args = list(big.mark = (",")), align = c('c','c','c'), caption = "Summary of Officer Data")
```
...
myData %>% 
    ...
    na.omit() %>%
      ...
      scale_fill_manual("Officer Gender", values = c("Female" = "green", "Male" = "lightblue")) +
      theme_bw()
```
...
officersArranged %>% 
  knitr::kable(col.names = c("Officer Race", "Gender", "Average Years of Service"), caption = "Officer Data by Average Years of Service", format.args = list(big.mark = ","), align = c('c','c','c'))
```
  ...
  filter(Officer_Race != "Not Specified") %>%
    na.omit() %>%
      group_by(CMPD_Division) %>% 
        count %>% 
          arrange(desc(n))  %>% 
            knitr::kable(col.names = c("CMPD Division", "Count"), format.args = list(big.mark = (",")), align = c('c','c'), caption = "Summary of Officer Data by CMPD Division")
```
